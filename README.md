...
| [![PPSSPP](https://img.shields.io/badge/PPSSPP-5B6DCD?style=flat-square)](https://github.com/hrydgard/ppsspp/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Three fixes: the Win32 debugger's breakpoint list refreshes after memcheck changes, its VFPU register view shows values and scrolls, and the folder buttons open again on Linux |
...
| [![Fyne](https://img.shields.io/badge/Fyne-1A9FD9?style=flat-square)](https://github.com/fyne-io/fyne/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Three fixes: a nil panic when a file dialog is sized before `Show()`, rasters drawn from the top left of their bounds, and submenus taller than the window now scroll |
...
| [![Throne](https://img.shields.io/badge/Throne-1A73E8?style=flat-square&logo=v2fly&logoColor=white)](https://github.com/throneproj/Throne/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Five fixes: a core crash on Remove Invalid, the system proxy clearing itself on exit, stale results from a finished latency sweep, screen-reader names in the hotkey settings, and a routing dialog that grew as wide as the longest profile name |
...
Across all of this I have opened pull requests against 51 projects that are not
mine. Another 53 of them are still open, in 34 projects including Kubernetes,
VS Code, Moby, Ollama, Next.js, TensorFlow, Supabase, Tailscale, PowerToys,
OBS Studio, Notepad++ and both Arduino IDEs.
...
| [![PPSSPP](https://img.shields.io/badge/PPSSPP-5B6DCD?style=flat-square)](https://github.com/hrydgard/ppsspp/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Три правки: список точек останова в отладчике Win32 обновляется после изменения memcheck, регистры VFPU показывают значения и прокручиваются, кнопки открытия папок снова работают на Linux |
...
| [![Fyne](https://img.shields.io/badge/Fyne-1A9FD9?style=flat-square)](https://github.com/fyne-io/fyne/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Три правки: падение по nil при изменении размера файлового диалога до `Show()`, растр рисуется от левого верхнего угла своих границ, подменю выше окна прокручиваются |
...
| [![Throne](https://img.shields.io/badge/Throne-1A73E8?style=flat-square&logo=v2fly&logoColor=white)](https://github.com/throneproj/Throne/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Пять правок: падение ядра на Remove Invalid, системный прокси не сбрасывался при выходе, устаревшие результаты завершённой проверки задержек, имена полей в настройках хоткеев для экранных дикторов и диалог маршрутов, растягивавшийся по самому длинному имени профиля |
...
Всего я открывал пул-реквесты в 51 чужой проект. Ещё 53 из них сейчас открыты,
в 34 проектах, среди которых Kubernetes, VS Code, Moby, Ollama, Next.js,
TensorFlow, Supabase, Tailscale, PowerToys, OBS Studio, Notepad++ и обе среды
Arduino.
