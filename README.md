...
| [![RetroArch](https://img.shields.io/badge/RetroArch-000000?style=flat-square)](https://github.com/libretro/RetroArch/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Restored the widget scale override range on console and mobile builds, and made companion thumbnails scale smoothly |
| [![Xray-core](https://img.shields.io/badge/Xray--core-2B2B2B?style=flat-square)](https://github.com/XTLS/Xray-core/pull/6818) | Made geosite regexp rules check their required literals before running the regexp, 20-40x faster domain matching on lists like geolocation-!cn |
| [![croc](https://img.shields.io/badge/croc-2E7D32?style=flat-square)](https://github.com/schollz/croc/pull/1349) | A zip the receiver declined to overwrite is no longer extracted and deleted, also after a reconnect |
| [![Glances](https://img.shields.io/badge/Glances-4B8BBE?style=flat-square)](https://github.com/nicolargo/glances/pull/3745) | Stopped the sensors plugin from reconnecting to an unreachable hddtemp daemon on every refresh (2 s per refresh on Windows) and made it read its own config section again |
| [![FreeRDP](https://img.shields.io/badge/FreeRDP-CD0000?style=flat-square)](https://github.com/FreeRDP/FreeRDP/pull/13520) | Fixed the Media Foundation H.264 decoder on Windows: the first frame was dropped, every later one lagged a frame behind and the colours were read from the wrong planes |
| [![ImageMagick](https://img.shields.io/badge/ImageMagick-3B5998?style=flat-square)](https://github.com/ImageMagick/ImageMagick/pull/8975) | Stopped copying the whole MVG document into every graphic context, drawing large vector files up to 10x faster |
| [![kitty](https://img.shields.io/badge/kitty-1F1F1F?style=flat-square)](https://github.com/kovidgoyal/kitty/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Three changes: fuzzy search in the themes and fonts kittens no longer freezes on long queries (from 28 s to 0.2 ms), the diff kitten's word diff no longer builds gigabyte tables on long lines, and `save_as_session` reports an error for a directory |
...
| [![RetroArch](https://img.shields.io/badge/RetroArch-000000?style=flat-square)](https://github.com/libretro/RetroArch/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Вернул диапазон настройки масштаба виджетов на консольных и мобильных сборках и сделал плавное масштабирование миниатюр в companion |
| [![Xray-core](https://img.shields.io/badge/Xray--core-2B2B2B?style=flat-square)](https://github.com/XTLS/Xray-core/pull/6818) | Научил правила-регулярки geosite сначала проверять обязательные подстроки, сопоставление доменов в списках вроде geolocation-!cn стало в 20-40 раз быстрее |
| [![croc](https://img.shields.io/badge/croc-2E7D32?style=flat-square)](https://github.com/schollz/croc/pull/1349) | Архив, который получатель отказался перезаписывать, больше не распаковывается и не удаляется, в том числе после переподключения |
| [![Glances](https://img.shields.io/badge/Glances-4B8BBE?style=flat-square)](https://github.com/nicolargo/glances/pull/3745) | Плагин сенсоров больше не переподключается к недоступному hddtemp на каждом обновлении (2 с на обновление на Windows) и снова читает свою секцию конфига |
| [![FreeRDP](https://img.shields.io/badge/FreeRDP-CD0000?style=flat-square)](https://github.com/FreeRDP/FreeRDP/pull/13520) | Починил декодер H.264 через Media Foundation на Windows: первый кадр терялся, остальные отставали на кадр, а цвет брался не из тех плоскостей |
| [![ImageMagick](https://img.shields.io/badge/ImageMagick-3B5998?style=flat-square)](https://github.com/ImageMagick/ImageMagick/pull/8975) | Перестал копировать весь MVG-документ в каждый графический контекст, отрисовка больших векторных файлов до 10 раз быстрее |
| [![kitty](https://img.shields.io/badge/kitty-1F1F1F?style=flat-square)](https://github.com/kovidgoyal/kitty/pulls?q=is%3Apr+author%3A4RH1T3CT0R7+is%3Amerged) | Три правки: нечёткий поиск в киттенах тем и шрифтов больше не зависает на длинных запросах (было 28 с, стало 0,2 мс), пословный diff не строит гигабайтные таблицы на длинных строках, `save_as_session` сообщает об ошибке для папки |
